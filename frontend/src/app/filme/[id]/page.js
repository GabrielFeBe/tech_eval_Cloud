...
import { pegarAtor, pegarFilme } from "@/lib/utils";
...
const excluirFilme = async () => {
  ...
  return (
    <main className="min-h-screen flex justify-center items-center flex-col">
      ...
      <CardInfo currEdit={estaEditando} edit={setEstaEditando} excluir={excluirFilme}>
        {estaEditando ? 
        (
        ...
        <p className="text-base p-4">Está disponivel: {filme.estaDisponivel ? 'Sim' : 'Não' }</p>
          </>
        )
        }
      </CardInfo>
      ...
      <div>
      <SearchInput buttonSize={"w-[40px] h-[44px] top-[-12px] right-[3.5px]"} funcaoPesquisa={buscarAtores} inpputSize={"w-[300px] h-[51px]"}
       placeholder={"Pesquise por um filme para adicionar ao ator"} setTexto={setAtorNome} texto={atorNome} />
        ...
        {atoresArray.map((ator) => {
          return (
            <div key={ator.id} className="flex flex-col gap-4">
              ...
              { setOfIds.has(ator.id) ? <p>Relacionado</p> :
                <button onClick={( )=> relacionarFilmeAoAtor(ator)}>Relacionar</button>
              }
            </div>
          );
        })}
      ...
      {filme.atores &&
        filme.atores.map((ator) => {
          return (
            <Link key={ator.id} href={`/ator/${ator.id}`} className="bg-[#626d7b] p-1 rounded-md">
              {ator.nome}
            </Link>
          )
        })}
